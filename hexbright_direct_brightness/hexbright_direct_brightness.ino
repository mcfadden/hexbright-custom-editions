/* 
  ...
  Designed to allow easy access to any mode from off.
  
  Author: Ben McFadden https://github.com/mcfadden
  
  Version: 1.0
  
  Derived from Factory firmware for HexBright FLEX v2.4  Dec 6, 2012
...
void loop()
{
  ...
  case MODE_OFF:
    if (btnDown && !newBtnDown && (time-btnTime)>20)
      newMode = MODE_LOW;
    if (btnDown && !newBtnDown && (time-btnTime)>250)
      newMode = MODE_MED;
    if (btnDown && !newBtnDown && (time-btnTime)>750)
      newMode = MODE_HIGH;
    if (btnDown && newBtnDown && (time-btnTime)>5000)
